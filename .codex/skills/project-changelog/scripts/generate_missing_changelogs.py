...
def catalog_value(contents: str, key: str) -> str:
...
def notable_changes(files: list[str], subjects: list[str]) -> list[str]:
    ...
    if any(path.startswith("android/src/") for path in files):
        notes.append("updated Android toolkit implementation and tests")
...
def dependencies(tag: str) -> dict[str, str]:
    ...
    return {
        ...
        "Kover": catalog_value(catalog, "jetbrains-kover"),
        "Detekt": catalog_value(catalog, "detekt"),
        "ktlint": catalog_value(catalog, "ktlint"),
        "minSdk": catalog_value(catalog, "build-sdk-min"),
...
Document the build, dependency, and Android toolkit changes shipped in {release.version}.
...
|:-------|:-----------------------------|
| Android | Supported, `minSdk {deps["minSdk"]}`, `compileSdk {deps["compileSdk"]}` |
"""
